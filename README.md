...
(行をダブルクリックで詳細)。個体を選んで **「交配計画」** を押すと、同じ画面の下に
「この個体の相手」が出る。誰と掛ければいいかが確率つきで並ぶ。
...
**種族ごとに変えられる。** ライブラリで種族を選んで「名前の付け方」を押すと、
その種族だけ入れるステータスを決められる (水生生物は酸素を外す、など)。
決めなければ全種族共通の設定を使う。
...
ライブラリで個体を選んで「交配計画」を押すと開く。タブは 5 つ。

| タブ | 中身 |
|---|---|
| この個体の相手 | **選んだ個体**に合う相手を並べる |
| おすすめペア | 全部の組み合わせを総当たり |
