...
class SpeciesNamingDialog(tk.Toplevel):
    def __init__(self, parent, app, species_bp, species_name, stat_list=None):
        ...
        tk.Label(self, text="%s の名前に入れるステータス" % species_name,
                 bg=theme.BG, fg=theme.INK,
                 font=theme.F.get("cute_b")).pack(anchor="w", padx=20, pady=(16, 2))
        tk.Label(self, text="この種族だけの決め方です。決めなければ"
                            "「取り込み通知」の設定 (全種族共通) が使われます。",
        ...
        self.status = tk.Label(
            self, text=("いまはこの種族だけの設定になっています"
                        if has_own else "いまは全種族共通の設定を使っています"),
            bg=theme.BG, fg=theme.INK_SUB, font=theme.F.get("small"))
        self.status.pack(anchor="w", padx=20, pady=(6, 0))

        box = tk.Frame(self, bg=theme.BG)
        box.pack(padx=20, pady=14)
        theme.RoundButton(box, "この種族に設定", self._save, kind="primary",
                          bg=theme.BG).pack(side="left", padx=4)
        theme.RoundButton(box, "共通の設定に戻す", self._clear, kind="soft",
                          bg=theme.BG).pack(side="left", padx=4)
    ...
    def _preview(self):
        ...
        self.preview.configure(text="こうなります:  " + text)

    def _save(self):
        chosen = self._chosen()
        if not chosen:
            self.status.configure(text="ひとつ以上選んでください")
            return
        self.st.library.set_setting("naming_stats_%s" % self.species_bp, chosen)
        self.status.configure(text="この種族だけの設定にしました")
